...
puts "These files were installed to %s: %s" % [ images_dir, Dir["#{images}/*"].map {|x| File.basename(x) }.join(", ") ]
...
puts "These files were installed to %s: %s" % [ stylesheets_dir, File.basename(css_file) ]